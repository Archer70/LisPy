...
def _print_bdd_summary(results: List[Dict[str, Any]]) -> Tuple[int, int, int, int, int]:
    ...
    for feature_idx, feature in enumerate(results):
        ...
        if not feature_has_failures:
            print(
                f"{EMOJI_PASS} Feature {feature_idx + 1}: {feature_description} ... PASSED ({len(scenarios_in_feature)} scenarios, {current_feature_total_steps} steps)"
            )
        else:
            print(
                f"{EMOJI_FAIL} Feature {feature_idx + 1}: {feature_description} ... FAILED"
            )
            ...
            for scenario_idx, scenario in enumerate(scenarios_in_feature):
                ...
                steps_in_scenario = scenario.get("steps", [])
                scenario_has_failures = any(
                    s.get("status", "unknown").upper() != "PASSED"
                    for s in steps_in_scenario
                )
                num_steps_in_scenario = len(steps_in_scenario)

                if not scenario_has_failures:
                    print(
                        f"  {EMOJI_PASS} Scenario {scenario_idx + 1}: {scenario_description} ... PASSED ({num_steps_in_scenario} steps)"
                    )
                else:
                    print(
                        f"  {EMOJI_FAIL} Scenario {scenario_idx + 1}: {scenario_description} ... FAILED"
                    )
                    ...
                    for step_idx, step in enumerate(steps_in_scenario):
                        ...
                        if status != "PASSED":
                            ...
                            print(
                                f"    {EMOJI_FAIL} {step_idx + 1}. {keyword} {description} ... {status}"
                            )
                            if details:
...
def run_bdd_tests(
    ...
    if not all_test_files:
        ...
        _print_bdd_summary(
            registry.BDD_RESULTS
        )  # Print empty summary (will say no tests found)
    ...
    for file_path in all_test_files:
        ...
        if return_code != 0:
            ...
            print(
                f"  {EMOJI_ERROR} CRITICAL ERROR executing BDD file {file_path}. Execution might be incomplete."
            )
    ...
    if failed_steps_from_summary > 0 or files_with_errors > 0:
        ...
        print(
            f"Overall BDD Test Run Status: {files_with_errors} file(s) with critical errors, {failed_steps_from_summary} failed steps."
        )
        return False
    else:

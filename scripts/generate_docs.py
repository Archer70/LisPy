...
def main():
    ...
    parser.add_argument('--preview', nargs='*', help='Generate docs for specific functions only')
    parser.add_argument('--all', action='store_true', help='Generate docs for all available functions and special forms')
    ...
    generator = HTMLGenerator(Path(args.output), project_root / 'docs' / 'templates', project_root)
    
    try:
        if args.all:
            # Generate all available documentation
            print("Generating documentation for all available functions and special forms...")
        ...
        if args.serve:
            ...
            for attempt in range(max_attempts):
                try:
                    with socketserver.TCPServer(("", port), http.server.SimpleHTTPRequestHandler) as httpd:
                        ...
                        print(f"📊 Total functions documented: {len(docs) if 'docs' in locals() else 'Unknown'}")
                        print()
                        print("💡 Tips:")
                        print("   • The server is running in the foreground (this is normal)")
                        print("   • Open http://localhost:{} in your browser".format(port))
                        print("   • Try the search functionality with terms like 'map', '+', or 'filter'")
            ...
            print("Please close other applications using these ports or specify a different port.")

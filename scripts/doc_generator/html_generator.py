...
class HTMLGenerator:
    ...
    def generate_function_page(
        self, func_doc: FunctionDoc, all_docs: Dict[str, FunctionDoc]
    ):
    ...
    def render_function_template(
        ...
        if func_doc.notes:
            notes_html = (
                "<ul>"
                + "".join(f"<li>{note}</li>" for note in func_doc.notes)
                + "</ul>"
            )
        ...
        if func_doc.see_also:
            see_also_html = ", ".join(
                f'<a href="../{ref}.html">{ref}</a>' for ref in func_doc.see_also
            )
        ...
        if special_forms:
            ...
            for form in sorted(special_forms, key=lambda f: f.name):
                ...
                if func_doc.type == "special-form":
                    ...
                    link_path = (
                        f"../../special-forms/{self.safe_filename(form.name)}.html"
                    )
                form_links += (
                    f'<li{active_class}><a href="{link_path}">{form.symbol}</a></li>\n'
                )
        ...
        for category, funcs in sorted(categories.items()):
            ...
            for func in sorted(funcs, key=lambda f: f.name):
                ...
                if func_doc.type == "special-form":
                    link_path = (
                        f"../functions/{category}/{self.safe_filename(func.name)}.html"
                    )
                else:
                    link_path = f"../../functions/{category}/{self.safe_filename(func.name)}.html"
                func_links += (
                    f'<li{active_class}><a href="{link_path}">{func.symbol}</a></li>\n'
                )

...
class DocumentationScanner:
    ...
    def scan_specific_functions(
        self, function_names: List[str]
    ) -> Dict[str, FunctionDoc]:
    ...
    def scan_function_docs(self, function_exports: List[str]) -> Dict[str, FunctionDoc]:
        ...
        try:
            ...
            for symbol in global_env.store.keys():
                ...
                if symbol not in ["nil", "true", "false"] and not symbol.startswith(
                    "__"
                ):
                ...
                try:
                    if symbol in DOCUMENTATION_REGISTRY:
                        ...
                        func_doc = self.parser.parse_function_doc(
                            doc_string, symbol, category, f"lispy/functions/{category}"
                        )
                        docs[symbol] = func_doc

                except Exception as e:
                    print(
                        f"Warning: Could not get documentation for function '{symbol}': {e}"
                    )
    ...
    def get_category_from_doc_function(self, doc_function) -> str:
        ...
        try:
            ...
            if (
                len(path_parts) >= 3
                and path_parts[0] == "lispy"
                and path_parts[1] == "functions"
            ):
    ...
    def scan_special_form_docs(
        self, special_form_exports: List[str]
    ) -> Dict[str, FunctionDoc]:
        ...
        try:
            ...
            for symbol in special_form_handlers.keys():
                try:
                    ...
                    print(
                        f"Warning: Could not get documentation for special form '{symbol}': {e}"
                    )

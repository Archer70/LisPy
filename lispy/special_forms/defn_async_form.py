...
def handle_defn_async_form(expression: List[Any], env: Environment, evaluate_fn: Callable) -> None:
    """
    ...
    """
    if len(expression) < 4:
        raise EvaluationError("SyntaxError: 'defn-async' expects at least 3 arguments (name, params, body...), got {}.".format(len(expression) - 1))
    ...
    if not isinstance(name, Symbol):
        raise EvaluationError("TypeError: 'defn-async' function name must be a symbol, got {}.".format(type(name).__name__))
    
    # Validate parameters (accept both lists and vectors)
    if not isinstance(params, (list, tuple)):
        raise EvaluationError("TypeError: 'defn-async' parameters must be a list or vector, got {}.".format(type(params).__name__))
    
    for param in params:
        if not isinstance(param, Symbol):
            raise EvaluationError("TypeError: 'defn-async' parameter must be a symbol, got {}.".format(type(param).__name__))

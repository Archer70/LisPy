...
from ..module_system import get_module_loader, set_current_module
    ...
    while i < len(expression):
        ...
        if isinstance(arg, Symbol) and arg.name == ":as":
            ...
            if not isinstance(prefix_expr, str):
                raise EvaluationError("SyntaxError: 'import :as' prefix must be a string")
            prefix = prefix_expr
            import_style = "prefixed"
            i += 2
        elif isinstance(arg, Symbol) and arg.name == ":only":
            if i + 1 >= len(expression):
                raise EvaluationError("SyntaxError: 'import :only' requires a symbol list")
            only_expr = expression[i + 1]
            if not isinstance(only_expr, list):
                raise EvaluationError("SyntaxError: 'import :only' requires a list of symbols")
            
            # Validate that all items in the list are symbols
            only_symbols = []
            for sym_expr in only_expr:
                if not isinstance(sym_expr, Symbol):
                    raise EvaluationError("SyntaxError: 'import :only' list must contain only symbols")
...
def _import_selective(module, env, symbol_names):
    ...
    for symbol_name in symbol_names:
        try:
            ...
            raise EvaluationError(f"Cannot import '{symbol_name}' from module '{module.name}': {str(e)}") 
...
def handle_await_form(expression: List[Any], env: Environment, evaluate_fn: Callable) -> Any:
    """
    ...
    """
    if len(expression) != 2:
        raise EvaluationError("SyntaxError: 'await' expects exactly 1 argument (promise expression), got {}.".format(len(expression) - 1))
    
    promise_expr = expression[1]
    promise = evaluate_fn(promise_expr, env)
    
    if not isinstance(promise, LispyPromise):
        raise EvaluationError("TypeError: 'await' can only be used with promises, got {}.".format(type(promise).__name__))

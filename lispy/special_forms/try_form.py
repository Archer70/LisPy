...
def handle_try_form(expression: List[Any], env: Environment, evaluate_fn: Callable) -> Any:
    """
    ...
    """
    if len(expression) < 2:
        raise EvaluationError("SyntaxError: 'try' expects at least 1 argument (body), got {}.".format(len(expression) - 1))
    ...
    for clause in expression[2:]:
        if not isinstance(clause, list) or len(clause) < 1:
            raise EvaluationError("SyntaxError: 'try' clauses must be lists starting with 'catch' or 'finally'.")
        
        clause_type = clause[0]
        if isinstance(clause_type, Symbol):
            if clause_type.name == "catch":
                if catch_clause is not None:
                    raise EvaluationError("SyntaxError: 'try' can only have one 'catch' clause.")
                ...
                if finally_clause is not None:
                    raise EvaluationError("SyntaxError: 'try' can only have one 'finally' clause.")
                # Validate finally clause syntax upfront
                _validate_finally_clause(clause)
                finally_clause = clause
            else:
                raise EvaluationError("SyntaxError: 'try' clauses must start with 'catch' or 'finally', got '{}'.".format(clause_type.name))
        else:
            raise EvaluationError("SyntaxError: 'try' clauses must start with 'catch' or 'finally'.")
...
def _validate_catch_clause(catch_clause: List[Any]) -> None:
    ...
    if len(catch_clause) < 3:
        raise EvaluationError("SyntaxError: 'catch' expects at least 2 arguments (binding handler-body...), got {}.".format(len(catch_clause) - 1))
    
    error_binding = catch_clause[1]
    if not isinstance(error_binding, Symbol):
        raise EvaluationError("SyntaxError: 'catch' binding must be a symbol, got {}.".format(type(error_binding).__name__))


def _validate_finally_clause(finally_clause: List[Any]) -> None:
    """Validate finally clause syntax."""
    if len(finally_clause) < 2:
        raise EvaluationError("SyntaxError: 'finally' expects at least 1 argument (cleanup-body), got {}.".format(len(finally_clause) - 1))


def _handle_catch_clause(catch_clause: List[Any], exception: Exception, env: Environment, evaluate_fn: Callable) -> Any:
...
def _handle_finally_clause(finally_clause: List[Any], env: Environment, evaluate_fn: Callable) -> None:

...
def builtin_assoc(args):
    ...
    """
    if len(args) < 3:
        raise EvaluationError(f"SyntaxError: 'assoc' expects at least 3 arguments (map, key, value), got {len(args)}.")

    original_map = args[0]
    kv_pairs = args[1:]

    if len(kv_pairs) % 2 != 0:
        raise EvaluationError(f"SyntaxError: 'assoc' requires an even number of key/value arguments after the map, got {len(kv_pairs)}.")

    if not (isinstance(original_map, dict) or original_map is None):
        raise EvaluationError(f"TypeError: First argument to 'assoc' must be a map or nil, got {type(original_map)}.")
    ...
    for i in range(0, len(kv_pairs), 2):
        ...
        if not isinstance(key, Symbol):
            raise EvaluationError(f"TypeError: Map keys in 'assoc' must be symbols, got {type(key)}.")

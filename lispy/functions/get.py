...
def get_fn(args):
    ...
    """
    if not 2 <= len(args) <= 3:
        raise EvaluationError(f"SyntaxError: 'get' expects 2 or 3 arguments, got {len(args)}.")
    ...
    if isinstance(collection, Vector):
        if not isinstance(key, int):
            raise EvaluationError(f"TypeError: Vector index must be an integer, got {type(key)}.")
        if 0 <= key < len(collection):
            return collection[key]
        elif default_value_provided:
            return default_value
        else:
            raise EvaluationError(f"IndexError: {key} out of bounds for vector of size {len(collection)}.")
    elif isinstance(collection, dict):  # Assuming maps are Python dicts
        if not isinstance(key, Symbol):
            raise EvaluationError(f"TypeError: Map key must be a symbol, got {type(key)}.")
        ...
        raise EvaluationError(f"TypeError: 'get' first argument must be a vector or map, got {type(collection)}.") 
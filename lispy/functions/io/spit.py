...
def builtin_spit(args: List[Any], env: Environment) -> None:
    ...
    if len(args) != 2:
        raise EvaluationError("SyntaxError: 'spit' expects 2 arguments (filename, content), got {}.".format(len(args)))
    
    filename_arg, content_arg = args
    
    # Validate filename is a string
    if not isinstance(filename_arg, str):
        raise EvaluationError("TypeError: 'spit' filename must be a string, got {}.".format(type(filename_arg).__name__))
    
    # Validate content is a string
    if not isinstance(content_arg, str):
        raise EvaluationError("TypeError: 'spit' content must be a string, got {}.".format(type(content_arg).__name__))
    ...
    try:
        ...
        if directory and not os.path.exists(directory):
            try:
                ...
                raise EvaluationError("OSError: Cannot create directory '{}': {}.".format(directory, str(e)))
        ...
        raise EvaluationError("PermissionError: Permission denied writing to file '{}'.".format(filename))
    except IsADirectoryError:
        raise EvaluationError("IsADirectoryError: '{}' is a directory, not a file.".format(filename))
    except OSError as e:
        raise EvaluationError("OSError: Error writing to file '{}': {}.".format(filename, str(e)))
    except Exception as e:
        raise EvaluationError("Error: Unexpected error writing to file '{}': {}.".format(filename, str(e)))

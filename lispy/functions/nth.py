...
def nth_fn(args: List[Any], env: Environment):
    ...
    """
    if not 2 <= len(args) <= 3:
        raise EvaluationError(f"SyntaxError: 'nth' expects 2 or 3 arguments, got {len(args)}.")
    ...
    if not isinstance(collection, (Vector, LispyList)):
        raise EvaluationError(f"TypeError: 'nth' first argument must be a vector or list, got {type(collection)}.")
    
    if not isinstance(index, int):
        raise EvaluationError(f"TypeError: 'nth' index must be an integer, got {type(index)}.")
    ...
    if 0 <= index < len(collection):
        ...
        raise EvaluationError(f"IndexError: {index} out of bounds for collection of size {len(collection)}.") 
...
def reverse_fn(args, env):
    ...
    """
    if len(args) != 1:
        raise EvaluationError(f"SyntaxError: 'reverse' expects 1 argument, got {len(args)}.")
    
    collection = args[0]
    
    # Validate collection type
    if not isinstance(collection, (LispyList, Vector)):
        raise EvaluationError(f"TypeError: Argument to 'reverse' must be a list or vector, got {type(collection)}.")

...
def builtin_http_request(args, env):
    ...
    if not (2 <= len(args) <= 4):
        raise EvaluationError("http-request expects 2-4 arguments (method url [data] [headers])")
    ...
    def http_request_handler():
        ...
        try:
            ...
            if not isinstance(method, str):
                promise.reject(HTTPError(f"Method must be a string, got {type(method).__name__}"))
                return
            ...
            common_methods = {'GET', 'POST', 'PUT', 'DELETE', 'HEAD', 'OPTIONS', 'PATCH', 'TRACE', 'CONNECT'}
            ...
            # Create request
            request = urllib.request.Request(url, data=request_body, headers=normalized_headers, method=method)

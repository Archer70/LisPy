"""
HTTP DELETE function for LisPy.
"""
...
def http_delete(args, env):
    ...
    if not (1 <= len(args) <= 3):
        raise EvaluationError("http-delete expects 1-3 arguments (url [data] [headers])")
    ...
    def http_request_handler():
        ...
        try:
            ...
            # Prepare request body (optional for DELETE)
            request_body = None
            ...
            # Create request
            request = urllib.request.Request(url, data=request_body, headers=normalized_headers, method='DELETE')
...
Examples:
  ...
  ; DELETE with resource ID in URL
  (http-delete "https://api.example.com/users/123")
  
  ; DELETE with custom headers
  (http-delete "https://api.example.com/resource/456"
               nil
               {:authorization "Bearer token123"})
  
  ; DELETE with data (uncommon but supported)
  (http-delete "https://api.example.com/bulk-delete"
               {:ids [1 2 3 4 5]})
...
Notes:
  ...
  - DELETE typically doesn't include a body, but it's supported for APIs that require it
  - Common status codes: 200 (OK), 204 (No Content), 404 (Not Found)
""" 
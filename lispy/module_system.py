...
class Module:
    ...
    def get_exported_value(self, symbol_name: str):
        ...
        if symbol_name not in self.exports:
            raise EvaluationError(f"Symbol '{symbol_name}' is not exported by module '{self.name}'")
...
class ModuleLoader:
    ...
    def load_module(self, module_name: str, evaluator_func) -> Module:
        ...
        if module_name in self.loading:
            raise EvaluationError(f"Circular dependency detected: module '{module_name}' is already being loaded")
            
        # Find the module file
        file_path = self.find_module_file(module_name)
        if file_path is None:
            raise EvaluationError(f"Module '{module_name}' not found in load paths: {self.load_paths}")

...
def end_feature() -> None:
    ...
    if _active_features_stack:
        _active_features_stack.pop()  # Remove the feature but don't store it
        # Note: feature info could be used for logging if needed
    else:
...
def start_scenario(description: str) -> None:
    ...
    if not current_feature:
        ...
        raise Exception(
            "INTERNAL ERROR: start_scenario called without an active feature."
        )
...
def end_scenario() -> None:
    ...
    if _active_scenarios_stack:
        _active_scenarios_stack.pop()  # Remove the scenario but don't store it
        # Note: scenario info could be used for logging if needed
    else:
...
def mark_last_step_status(new_status: str, details: Optional[str] = None) -> None:
    ...
    if not current_scenario:
        print(
            "[REGISTRY_WARNING] mark_last_step_status called without an active scenario."
        )
        return

    if not current_scenario["steps"]:
        print(
            "[REGISTRY_WARNING] mark_last_step_status called but no steps in current scenario."
        )
        return

...
class MiddlewareChain:
    ...
    def add_middleware(self, middleware_type: str, handler: Callable) -> None:
        ...
        if middleware_type not in ['before', 'after']:
            raise ValueError(f"Invalid middleware type: {middleware_type}. Must be 'before' or 'after'.")
    ...
    def execute_before_middleware(self, request: Dict[Symbol, Any], env) -> Dict[Symbol, Any]:
        """
        ...
        for middleware in self.middleware:
            if middleware.middleware_type == 'before':
                try:
                    ...
                    result = self._execute_middleware_function(middleware.handler, current_request, env)
                    
                    # Middleware should return modified request or None (no change)
                    if result is not None:
                        if not isinstance(result, dict):
                            raise EvaluationError(f"Before middleware must return a request object (dict), got {type(result).__name__}")
    ...
    def execute_after_middleware(self, request: Dict[Symbol, Any], response: Dict[Symbol, Any], env) -> Dict[Symbol, Any]:
        """
        ...
        for middleware in self.middleware:
            if middleware.middleware_type == 'after':
                try:
                    ...
                    result = self._execute_middleware_function(middleware.handler, request, env, current_response)
                    
                    # Middleware should return modified response or None (no change)
                    if result is not None:
                        if not isinstance(result, dict):
                            raise EvaluationError(f"After middleware must return a response object (dict), got {type(result).__name__}")
    ...
    def _execute_middleware_function(self, handler: Callable, request: Dict[Symbol, Any], env, response: Dict[Symbol, Any] = None) -> Any:
        """
        ...
        if isinstance(handler, Function):
            ...
            if response is not None:
                ...
                if len(handler.params) != 2:
                    raise EvaluationError(f"After middleware function must take 2 arguments (request response), got {len(handler.params)}")
                ...
                if len(handler.params) != 1:
                    raise EvaluationError(f"Before middleware function must take 1 argument (request), got {len(handler.params)}")
            ...
            raise EvaluationError(f"Middleware handler must be a function, got {type(handler).__name__}")

...
from .response import format_response, create_error_response, create_method_not_allowed_response
...
class WebApp:
    ...
    def handle_request(self, method: str, path: str, headers, body: str = "", 
                      client_address = None, env = None):
        """
        ...
        try:
            ...
            if not isinstance(response, dict):
                return create_error_response(500, f"Route handler must return a response object (dict), got {type(response).__name__}")
    ...
    def _execute_route_handler(self, handler: Callable, request: Dict[Symbol, Any], env) -> Dict[Symbol, Any]:
        """
        ...
        if isinstance(handler, Function):
            ...
            raise EvaluationError(f"Route handler must be a function, got {type(handler).__name__}")

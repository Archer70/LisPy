...
from lispy.lexer import (
    TOKEN_NUMBER, TOKEN_STRING, TOKEN_BOOLEAN, TOKEN_NIL, TOKEN_SYMBOL,
    TOKEN_LPAREN, TOKEN_RPAREN, TOKEN_QUOTE,
...
class ParserTest(unittest.TestCase):
    ...
    def test_parse_empty(self):
        with self.assertRaisesRegex(ParseError, "Unexpected end of input: No tokens to parse."):
            parse([])
    ...
    def test_parse_nested_lists(self):
        tokens = [
            (TOKEN_LPAREN, '('), (TOKEN_SYMBOL, '+'), (TOKEN_NUMBER, '1'),
            (TOKEN_LPAREN, '('), (TOKEN_SYMBOL, '*'), (TOKEN_NUMBER, '2'), (TOKEN_NUMBER, '3'), (TOKEN_RPAREN, ')'),
            (TOKEN_RPAREN, ')')
        ]
    ...
    def test_parse_quote_shorthand_nested(self):
        tokens = [
            (TOKEN_QUOTE, "'"), (TOKEN_LPAREN, '('), (TOKEN_SYMBOL, "a"),
            (TOKEN_QUOTE, "'"), (TOKEN_LPAREN, '('), (TOKEN_SYMBOL, "b"), (TOKEN_SYMBOL, "c"), (TOKEN_RPAREN, ')'),
            (TOKEN_RPAREN, ')')
        ]
        expected_ast = [
            Symbol("quote"), 
    ...
    def test_parse_quote_shorthand_eof_error(self):
        tokens = [(TOKEN_QUOTE, "'")]
        with self.assertRaisesRegex(ParseError, "SyntaxError: 'quote' shorthand ' must be followed by an expression."):
            parse(tokens)
    ...
    def test_parse_vector_with_nested_vector(self):
        tokens = [
            (TOKEN_LBRACKET, '['),
            (TOKEN_NUMBER, "1"),
            (TOKEN_LBRACKET, '['), (TOKEN_NUMBER, '2'), (TOKEN_NUMBER, '3'), (TOKEN_RBRACKET, ']'),
            (TOKEN_NUMBER, "4"),
            (TOKEN_RBRACKET, ']')
        ]
    ...
    def test_parse_unclosed_vector(self):
        ...
        with self.assertRaisesRegex(ParseError, "Unexpected end of input: missing ']' while parsing vector"):
            parse(tokens)
    ...
    def test_parse_vector_unexpected_token(self):
        ...
        with self.assertRaisesRegex(ParseError, "Unexpected token type during parsing: RPAREN"):
            parse(tokens)
    ...
    def test_parse_map_with_symbol_keys_atom_values(self):
        tokens = [
            (TOKEN_LBRACE, '{'),
            (TOKEN_SYMBOL, ":a"), (TOKEN_NUMBER, "1"),
            (TOKEN_SYMBOL, ":b"), (TOKEN_STRING, "two"),
            (TOKEN_SYMBOL, ":c"), (TOKEN_BOOLEAN, True),
            (TOKEN_RBRACE, '}')
        ]
        expected_map = {
            Symbol(":a"): 1,
    ...
    def test_parse_map_with_nested_values(self):
        tokens = [
            (TOKEN_LBRACE, '{'),
            (TOKEN_SYMBOL, ":l"), (TOKEN_LPAREN, '('), (TOKEN_NUMBER, "1"), (TOKEN_NUMBER, "2"), (TOKEN_RPAREN, ')'),
            (TOKEN_SYMBOL, ":v"), (TOKEN_LBRACKET, '['), (TOKEN_STRING, "x"), (TOKEN_BOOLEAN, False), (TOKEN_RBRACKET, ']'),
            (TOKEN_RBRACE, '}')
        ]
        expected_map = {
            Symbol(":l"): [1, 2],
    ...
    def test_parse_unclosed_map(self):
        ...
        with self.assertRaisesRegex(ParseError, "Unexpected end of input: missing '}' while parsing map"):
            parse(tokens)
    ...
    def test_parse_map_odd_number_of_forms(self):
        ...
        with self.assertRaisesRegex(ParseError, "Map literals require an even number of forms \\(key-value pairs\\), missing value for key: :b"):
            parse(tokens)
    ...
    def test_parse_map_odd_number_of_forms_eof(self):
        ...
        with self.assertRaisesRegex(ParseError, "Unexpected end of input: map literal requires a value for key: :b"):
            parse(tokens)

    def test_parse_map_non_symbol_key(self):
        tokens = [
            (TOKEN_LBRACE, '{'),
            (TOKEN_NUMBER, "123"), (TOKEN_STRING, "value"),
            (TOKEN_RBRACE, '}')
        ]
    ...
    def test_parse_map_non_symbol_key_string(self):
        tokens = [
            (TOKEN_LBRACE, '{'),
            (TOKEN_STRING, "key"), (TOKEN_STRING, "value"),
            (TOKEN_RBRACE, '}')
        ]
    ...
    def test_parse_extra_rparen_after_list(self):
        ...
        with self.assertRaisesRegex(ParseError, r"Unexpected tokens at end of input.*RPAREN"):
            parse(tokens)
    ...
    def test_parse_extra_rbrace_after_map(self):
        ...
        with self.assertRaisesRegex(ParseError, r"Unexpected tokens at end of input.*RBRACE"):
            parse(tokens)

    def test_parse_unclosed_inner_list(self):
        tokens = [(TOKEN_LPAREN, '('), (TOKEN_LPAREN, '('), (TOKEN_RPAREN, ')')]
        with self.assertRaisesRegex(ParseError, r"Unexpected end of input.*missing.*while parsing list"):
            parse(tokens)
    ...
    def test_parse_unexpected_rbracket_at_start(self):
        ...
        with self.assertRaisesRegex(ParseError, "Unexpected token type during parsing: RBRACKET \\(\\'\\]\\'\\)"):
            parse(tokens)

    def test_parse_unexpected_rbrace_at_start(self):
        tokens = [(TOKEN_RBRACE, '}')]
        with self.assertRaisesRegex(ParseError, r"Unexpected token type during parsing: RBRACE \('.*'\)"):
            parse(tokens)

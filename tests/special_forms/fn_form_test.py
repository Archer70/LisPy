...
class FnFormTest(unittest.TestCase):  # Renamed class
    ...
    def test_fn_closure_captures_defining_env(self):  # Renamed test
        ...
        # (add5 3) -> 8
        run_lispy_string(
            "(define n_outer 100)", self.env
        )  # Should not be used by add5 if closure is correct
    ...
    def test_fn_recursion_via_define(self):  # Renamed test
        run_lispy_string(
            "(define fact (fn [n] (if (<= n 1) 1 (* n (fact (- n 1))))))", self.env
        )
    ...
    def test_fn_syntax_error_params_not_list(self):  # Renamed test
        ...
        self.assertTrue(
            "SyntaxError: Parameter list for 'fn' must be a list" in str(cm.exception)
        )
    ...
    def test_fn_syntax_error_param_not_symbol(self):  # Renamed test
        ...
        self.assertTrue(
            "SyntaxError: All parameters in 'fn' parameter list must be symbols"
            in str(cm.exception)
        )
        with self.assertRaises(EvaluationError) as cm:

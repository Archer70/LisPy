...
class TestOrForm(unittest.TestCase):
    ...
    def test_or_all_false(self):
        ...
        self.assertEqual(self._run_lispy("(or nil nil nil)"), None) # Returns last value (nil)
    ...
    def test_or_evaluation_order_and_return_last_falsy(self):
        ...
        evaluate(parse(tokenize("(define y false)")), self.env) # Tokenize before parsing
        self.assertEqual(self._run_lispy("(or x y)"), False) # x (nil) then y (false), returns false
        self.assertEqual(self._run_lispy("(or y x)"), None)  # y (false) then x (nil), returns nil

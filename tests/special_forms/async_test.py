...
class TestAsyncSpecialForms(unittest.TestCase):
    ...
    def test_async_with_resolved_promise(self):
        ...
        result = handle_async_form([Symbol("async"), promise_expr], self.env, self.evaluate_fn)
    ...
    def test_async_wrong_arg_count(self):
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'async' expects exactly 1 argument (body expression), got 0.")
        
        with self.assertRaises(EvaluationError) as cm:
            handle_async_form([Symbol("async"), 1, 2], self.env, self.evaluate_fn)
        self.assertEqual(str(cm.exception), "SyntaxError: 'async' expects exactly 1 argument (body expression), got 2.")
    
    def test_await_with_resolved_promise(self):
        """Test await with a resolved promise."""
        promise = builtin_resolve([42], self.env)
        result = handle_await_form([Symbol("await"), promise], self.env, self.evaluate_fn)
        self.assertEqual(result, 42)
    ...
    def test_await_with_non_promise(self):
        ...
        self.assertEqual(str(cm.exception), "TypeError: 'await' can only be used with promises, got int.")
    ...
    def test_await_wrong_arg_count(self):
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'await' expects exactly 1 argument (promise expression), got 0.")
        
        promise = builtin_resolve([1], self.env)
        with self.assertRaises(EvaluationError) as cm:
            handle_await_form([Symbol("await"), promise, promise], self.env, self.evaluate_fn)
        self.assertEqual(str(cm.exception), "SyntaxError: 'await' expects exactly 1 argument (promise expression), got 2.")
    ...
    def test_defn_async_basic(self):
        ...
        result = handle_defn_async_form([Symbol("defn-async"), name, params] + body, self.env, self.evaluate_fn)
    ...
    def test_defn_async_no_params(self):
        ...
        handle_defn_async_form([Symbol("defn-async"), name, params] + body, self.env, self.evaluate_fn)
    ...
    def test_defn_async_wrong_arg_count(self):
        ...
        with self.assertRaises(EvaluationError) as cm:
            handle_defn_async_form([Symbol("defn-async"), Symbol("fn")], self.env, self.evaluate_fn)
        self.assertEqual(str(cm.exception), "SyntaxError: 'defn-async' expects at least 3 arguments (name, params, body...), got 1.")
        
        with self.assertRaises(EvaluationError) as cm:
            handle_defn_async_form([Symbol("defn-async"), Symbol("fn"), Vector([])], self.env, self.evaluate_fn)
        self.assertEqual(str(cm.exception), "SyntaxError: 'defn-async' expects at least 3 arguments (name, params, body...), got 2.")
    
    def test_defn_async_invalid_name(self):
        """Test defn-async with invalid function name."""
        with self.assertRaises(EvaluationError) as cm:
            handle_defn_async_form([Symbol("defn-async"), 42, Vector([]), 1], self.env, self.evaluate_fn)
        self.assertEqual(str(cm.exception), "TypeError: 'defn-async' function name must be a symbol, got int.")
    
    def test_defn_async_invalid_params(self):
        """Test defn-async with invalid parameters."""
        with self.assertRaises(EvaluationError) as cm:
            handle_defn_async_form([Symbol("defn-async"), Symbol("fn"), 42, 1], self.env, self.evaluate_fn)
        self.assertEqual(str(cm.exception), "TypeError: 'defn-async' parameters must be a list or vector, got int.")
        
        with self.assertRaises(EvaluationError) as cm:
            handle_defn_async_form([Symbol("defn-async"), Symbol("fn"), Vector([42]), 1], self.env, self.evaluate_fn)
        self.assertEqual(str(cm.exception), "TypeError: 'defn-async' parameter must be a symbol, got int.")
    ...
    def test_defn_async_wrong_call_args(self):
        ...
        with self.assertRaises(EvaluationError) as cm:
            async_fn([], self.env)
        self.assertEqual(str(cm.exception), "SyntaxError: 'test-fn' expects 1 arguments, got 0.")
        
        with self.assertRaises(EvaluationError) as cm:
            async_fn([1, 2], self.env)
        self.assertEqual(str(cm.exception), "SyntaxError: 'test-fn' expects 1 arguments, got 2.")

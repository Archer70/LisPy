...
class CondSpecialFormTest(unittest.TestCase):
    ...
    def test_cond_no_arguments(self):
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'cond' requires at least one test-result pair.")

    def test_cond_odd_number_of_arguments(self):
        """Test cond with odd number of arguments."""
        with self.assertRaises(EvaluationError) as cm:
            run_lispy_string('(cond true)', self.env)
        self.assertEqual(str(cm.exception), "SyntaxError: 'cond' requires an even number of arguments (test-result pairs).")
    ...
    def test_cond_with_error_in_condition(self):
        ...
        with self.assertRaises(EvaluationError):
            run_lispy_string('(cond (/ 1 0) "should not reach" true "fallback")', self.env)

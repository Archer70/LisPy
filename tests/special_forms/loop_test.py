...
class LoopSpecialFormTest(unittest.TestCase):
    ...
    def test_loop_only_bindings_no_body(self):
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'loop' requires a bindings vector and at least one body expression. Usage: (loop [var val ...] body...)")
    ...
    def test_loop_with_cond(self):
        ...
        (loop [x 15]
          (cond
            ...
            true (recur (- x 5))))
        ''', self.env)
        self.assertEqual(result, "medium")  # 15 -> 10 -> 5, and 5 is not < 5, so "medium"

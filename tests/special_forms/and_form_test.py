...
class TestAndForm(unittest.TestCase):
    ...
    def test_and_with_various_truthy_values(self):
        self.assertEqual(
            self._run_lispy('(and 0 "s" (list 1) (vector 2))'), [2]
        )  # Returns last vector object
        self.assertEqual(
            self._run_lispy("(and (fn (x) (+ x 1)) 1)"), 1
        )  # Function object is truthy

    def test_and_with_various_falsy_values(self):
        self.assertEqual(
            self._run_lispy("(and true nil false)"), None
        )  # nil encountered first
        self.assertEqual(
            self._run_lispy("(and true false nil)"), False
        )  # false encountered first

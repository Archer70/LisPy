...
class TestErrorHandling(unittest.TestCase):
    ...
    def test_throw_simple_value(self):
        ...
        with self.assertRaises(UserThrownError) as cm:
            handle_throw_form([Symbol("throw"), "test error"], self.env, self.evaluate_fn)
    ...
    def test_throw_evaluated_expression(self):
        ...
        with self.assertRaises(UserThrownError) as cm:
            handle_throw_form([Symbol("throw"), [Symbol("+"), 1, 2]], self.env, self.evaluate_fn)
    ...
    def test_throw_wrong_arg_count(self):
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'throw' expects exactly 1 argument (value), got 0.")
        
        with self.assertRaises(EvaluationError) as cm:
            handle_throw_form([Symbol("throw"), 1, 2], self.env, self.evaluate_fn)
        self.assertEqual(str(cm.exception), "SyntaxError: 'throw' expects exactly 1 argument (value), got 2.")
    ...
    def test_try_multiple_expressions_in_body(self):
        ...
        result = handle_try_form([
            Symbol("try"),
            [Symbol("throw"), "error"],
            [Symbol("catch"), Symbol("e"), 
             [Symbol("+"), 1, 2],  # First expression
             [Symbol("*"), 3, 4]]  # Second expression (should be returned)
        ], self.env, self.evaluate_fn)
    ...
    def test_try_invalid_syntax(self):
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'try' expects at least 1 argument (body), got 0.")
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'catch' expects at least 2 arguments (binding handler-body...), got 0.")
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'catch' binding must be a symbol, got int.")
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'try' can only have one 'catch' clause.")
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'try' can only have one 'finally' clause.")

...
class TestBddIntegration(unittest.TestCase):
    ...
    def test_full_bdd_flow_populates_registry(self):
        ...
        self.assertEqual(
            scenario1["description"], "should allow login with correct credentials"
        )
        self.assertEqual(len(scenario1["steps"]), 4)
        self.assertEqual(scenario1["steps"][0]["keyword"], "Given")
        self.assertEqual(
            scenario1["steps"][0]["description"],
            "a registered user 'testuser' with password 'pass123'",
        )
        ...
        self.assertEqual(
            scenario2["description"], "should deny login with incorrect credentials"
        )
    ...
    def test_nested_describes_not_supported_yet(self):
        ...
        self.assertEqual(
            len(registry.BDD_RESULTS[0]["scenarios"]), 0
        )  # Inner describe is not a scenario of outer
        self.assertEqual(
            registry.BDD_RESULTS[1]["description"],
            "Inner Feature - treated as new top-level",
        )

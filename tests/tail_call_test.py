...
from lispy.tail_call import (TailCall, is_function_call, is_recursive_call,
                             is_tail_position)
...
class IntegrationTest(unittest.TestCase):
    ...
    def test_tail_call_workflow(self):
        ...
        self.assertFalse(
            is_tail_position(0, len(body))
        )  # First expression not in tail position
        self.assertTrue(
            is_tail_position(1, len(body))
    ...
    def test_non_tail_recursive_call(self):
        ...
        self.assertTrue(
            is_tail_position(0, len(function_body))

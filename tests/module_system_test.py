...
import tempfile
import shutil
from lispy.module_system import ModuleLoader, Module, get_module_loader
...
class ExportFormTest(unittest.TestCase):
    ...
    def test_export_basic_functionality(self):
        ...
        with self.assertRaisesRegex(EvaluationError, "'export' can only be used within a module"):
            run_lispy_string("(export x y)", self.env)
            
    def test_export_requires_symbols(self):
        """Test that export requires at least one symbol."""
        with self.assertRaisesRegex(EvaluationError, "'export' requires at least one symbol"):

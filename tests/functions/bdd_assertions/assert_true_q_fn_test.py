...
class TestAssertTrueQFn(unittest.TestCase):
    ...
    def run_in_then_context(self, then_body_code: str):
        ...
        run_lispy_string(
            f'(then "a test assertion with assert-true?" {then_body_code})', self.env
        )
    ...
    def test_assert_true_q_fail_with_number(self):
        self.run_in_then_context(
            "(assert-true? 0)"
        )  # In many languages 0 is falsey, but not strictly True
        scenario = registry.BDD_RESULTS[0]["scenarios"][0]
        step = scenario["steps"][0]
        self.assertEqual(step["status"], "failed")
    ...
    def test_assert_true_q_arity_error_no_args(self):
        ...
        self.assertEqual(step["status"], "failed")
        self.assertIn(
            "Step error: SyntaxError: 'assert-true?' expects 1 argument",
            step["details"],
        )

...
class TestAssertNilQFn(unittest.TestCase):
    ...
    def run_in_then_context(self, then_body_code: str):
        ...
        run_lispy_string(f'(then "a test assertion with assert-nil?" {then_body_code})', self.env)
    ...
    def test_assert_nil_q_arity_error_no_args(self):
        ...
        step = scenario["steps"][0]
        self.assertEqual(step["status"], "failed")
        self.assertIn("Step error: SyntaxError: 'assert-nil?' expects 1 argument", step["details"])

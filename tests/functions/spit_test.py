...
class SpitFunctionTest(unittest.TestCase):
    ...
    def test_spit_no_arguments(self):
        ...
        self.assertIn("SyntaxError: 'spit' expects 2 arguments (filename, content), got 0.", str(cm.exception))

    def test_spit_one_argument(self):
        # Test with only one argument
        with self.assertRaises(Exception) as cm:
            run_lispy_string('(spit "filename.txt")', self.env)
        self.assertIn("SyntaxError: 'spit' expects 2 arguments (filename, content), got 1.", str(cm.exception))

    def test_spit_too_many_arguments(self):
        # Test with too many arguments
        with self.assertRaises(Exception) as cm:
            run_lispy_string('(spit "file1.txt" "content" "extra")', self.env)
        self.assertIn("SyntaxError: 'spit' expects 2 arguments (filename, content), got 3.", str(cm.exception))

    def test_spit_non_string_filename(self):
        # Test with non-string filename
        with self.assertRaises(Exception) as cm:
            run_lispy_string('(spit 42 "content")', self.env)
        self.assertIn("TypeError: 'spit' filename must be a string, got int.", str(cm.exception))

    def test_spit_non_string_content(self):
        # Test with non-string content
        with self.assertRaises(Exception) as cm:
            run_lispy_string('(spit "filename.txt" 42)', self.env)
        self.assertIn("TypeError: 'spit' content must be a string, got int.", str(cm.exception))

    def test_spit_boolean_arguments(self):
        # Test with boolean arguments
        with self.assertRaises(Exception) as cm:
            run_lispy_string('(spit true "content")', self.env)
        self.assertIn("TypeError: 'spit' filename must be a string, got bool.", str(cm.exception))

        with self.assertRaises(Exception) as cm:
            run_lispy_string('(spit "filename.txt" false)', self.env)
        self.assertIn("TypeError: 'spit' content must be a string, got bool.", str(cm.exception))
    ...
    def test_spit_integration_with_join(self):
        ...
        result = run_lispy_string(f'(spit "{filepath}" (join ["line1" "line2" "line3"] "\\n"))', self.env)
        self.assertIsNone(result)
    ...
    def test_spit_permission_error(self, mock_open):
        ...
        self.assertIn("PermissionError: Permission denied writing to file", str(cm.exception))

...
class IsFunctionQFnTest(unittest.TestCase):
    ...
    def test_function_q_builtin_function(self):
        """Test (is_function? +) returns true for built-in functions."""
        self.assertTrue(run_lispy_string("(is_function? +)", self.env))

    def test_function_q_user_defined_function(self):
        """Test that is_function? returns true for user-defined functions."""
        # Define a function first, then test it
        run_lispy_string("(define my-fn (fn [x] (* x 2)))", self.env)
        self.assertTrue(run_lispy_string("(is_function? my-fn)", self.env))

    def test_function_q_lambda_function(self):
        """Test that is_function? returns true for lambda functions."""
        self.assertTrue(run_lispy_string("(is_function? (fn [x] (+ x 1)))", self.env))
    ...
    def test_function_q_string(self):
        """Test (is_function? \"hello\") returns false."""
        self.assertFalse(run_lispy_string('(is_function? "hello")', self.env))

    def test_function_q_boolean_true(self):
        """Test (is_function? true) returns false."""
        self.assertFalse(run_lispy_string("(is_function? true)", self.env))

    def test_function_q_boolean_false(self):
        """Test (is_function? false) returns false."""
        self.assertFalse(run_lispy_string("(is_function? false)", self.env))

    def test_function_q_nil(self):
        """Test (is_function? nil) returns false."""
        self.assertFalse(run_lispy_string("(is_function? nil)", self.env))

    def test_function_q_list(self):
        """Test (is_function? '(1 2 3)) returns false."""
        self.assertFalse(run_lispy_string("(is_function? '(1 2 3))", self.env))

    def test_function_q_empty_list(self):
        """Test (is_function? '()) returns false."""
        self.assertFalse(run_lispy_string("(is_function? '())", self.env))
    ...
    def test_function_q_map(self):
        """Test (is_function? {:a 1}) returns false."""
        self.assertFalse(run_lispy_string("(is_function? {:a 1})", self.env))
    ...
    def test_function_q_symbol(self):
        """Test (is_function? 'x) returns false."""
        self.assertFalse(run_lispy_string("(is_function? 'x)", self.env))

    def test_function_q_with_partial_application(self):
        """Test that is_function? works with partially applied functions."""
        # Create a partial application and test it
        run_lispy_string("(define add-5 (fn [x] (+ x 5)))", self.env)
        self.assertTrue(run_lispy_string("(is_function? add-5)", self.env))

    def test_function_q_nested_function_definition(self):
        """Test that is_function? works with nested function definitions."""

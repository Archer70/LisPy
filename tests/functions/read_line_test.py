...
class ReadLineFunctionTest(unittest.TestCase):
    ...
    def test_read_line_too_many_arguments(self):
        ...
        self.assertIn("SyntaxError: 'read-line' expects 0 or 1 arguments (optional prompt), got 2.", str(cm.exception))

    def test_read_line_non_string_prompt(self):
        with self.assertRaises(Exception) as cm:
            run_lispy_string('(read-line 42)', self.env)
        self.assertIn("TypeError: 'read-line' prompt must be a string, got int.", str(cm.exception))

    def test_read_line_boolean_prompt(self):
        with self.assertRaises(Exception) as cm:
            run_lispy_string('(read-line true)', self.env)
        self.assertIn("TypeError: 'read-line' prompt must be a string, got bool.", str(cm.exception))

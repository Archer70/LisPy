...
class TestPromiseRace(unittest.TestCase):
    ...
    def test_promise_race_wrong_arg_count(self):
        ...
        self.assertEqual(
            str(cm.exception), "SyntaxError: 'promise-race' expects 1 argument, got 0."
        )

        with self.assertRaises(EvaluationError) as cm:
            promise_race([Vector([]), Vector([])], self.env)
        self.assertEqual(
            str(cm.exception), "SyntaxError: 'promise-race' expects 1 argument, got 2."
        )
    ...
    def test_promise_race_concurrent_resolution(self):
        ...
        self.assertLess(
            execution_time, 30, "Should complete in time of fastest promise"
        )

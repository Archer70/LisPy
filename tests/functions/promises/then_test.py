...
class TestPromiseThen(unittest.TestCase):
    ...
    def test_then_with_different_value_types(self):
        ...
        result_str = promise_then([promise_obj, lambda args, env: str(args[0])], self.env)
        self.assertEqual(result_str.value, "42")
        
        # String to vector
        promise_obj = resolve(["a,b,c"], self.env)
        result_vec = promise_then([promise_obj, lambda args, env: Vector(args[0].split(","))], self.env)
        self.assertEqual(result_vec.value, Vector(["a", "b", "c"]))
        
        # Vector to number
        promise_obj = resolve([Vector([1, 2, 3])], self.env)
        result_sum = promise_then([promise_obj, lambda args, env: sum(args[0])], self.env)

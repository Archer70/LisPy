...
class TestTimeoutFunctions(unittest.TestCase):
    ...
    def test_timeout_argument_validation(self):
        ...
        self.assertEqual(
            str(cm.exception),
            "SyntaxError: 'timeout' expects 1 or 2 arguments (ms [value]), got 0.",
        )
        ...
        self.assertEqual(
            str(cm.exception),
            "SyntaxError: 'timeout' expects 1 or 2 arguments (ms [value]), got 3.",
        )
        ...
        self.assertEqual(
            str(cm.exception),
            "TypeError: 'timeout' first argument (ms) must be a number, got str.",
        )
    ...
    def test_with_timeout_basic_functionality(self):
        ...
        result = run_lispy_string(
            '(with-timeout (resolve "fast") "fallback" 1000)', self.env
        )
        self.assertIsInstance(result, LispyPromise)
    ...
    def test_with_timeout_triggers_fallback(self):
        ...
        result = run_lispy_string(
            '(with-timeout (timeout 200 "slow") "fallback" 100)', self.env
        )
        self.assertIsInstance(result, LispyPromise)
    ...
    def test_with_timeout_preserves_rejection(self):
        ...
        result = run_lispy_string(
            '(with-timeout (reject "error") "fallback" 1000)', self.env
        )
        self.assertIsInstance(result, LispyPromise)
    ...
    def test_with_timeout_argument_validation(self):
        ...
        with self.assertRaises(EvaluationError) as cm:
            run_lispy_string(
                '(with-timeout (resolve 42) "fallback" 1000 "extra")', self.env
            )
        self.assertEqual(
            str(cm.exception),
        ...
        with self.assertRaises(EvaluationError) as cm:
            run_lispy_string(
                '(with-timeout (resolve 42) "fallback" "not-number")', self.env
            )
        self.assertEqual(
            str(cm.exception),

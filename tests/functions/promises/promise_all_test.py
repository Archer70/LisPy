...
class TestPromiseAll(unittest.TestCase):
    ...
    def test_promise_all_wrong_arg_count(self):
        ...
        self.assertEqual(
            str(cm.exception), "SyntaxError: 'promise-all' expects 1 argument, got 0."
        )

        with self.assertRaises(EvaluationError) as cm:
            promise_all([Vector([]), Vector([])], self.env)
        self.assertEqual(
            str(cm.exception), "SyntaxError: 'promise-all' expects 1 argument, got 2."
        )
    ...
    def test_promise_all_thread_safety(self):
        ...
        promises = Vector(
            [promise([concurrent_function(i)], self.env) for i in range(10)]
        )

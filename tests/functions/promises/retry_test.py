...
class TestRetry(unittest.TestCase):
    ...
    def test_retry_with_user_defined_function(self):
        ...
        run_lispy_string(
            '(define simple-success-op (fn [] "User-defined function works"))', self.env
        )
    ...
    def test_argument_validation_non_integer_attempts(self):
        ...
        self.assertIn(
            "second argument (max-attempts) must be an integer", str(cm.exception)
        )
    ...
    def test_concurrent_retry_operations(self):
        ...
        expected = Vector(
            ["Success on attempt 1", "Success on attempt 2", "Success on attempt 3"]
        )

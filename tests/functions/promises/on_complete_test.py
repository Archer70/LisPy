...
class TestOnComplete(unittest.TestCase):
    ...
    def test_on_complete_with_resolved_promise(self):
        ...
        result = run_lispy_string(
            '(on-complete (resolve "success") (fn [p] "cleanup done"))', self.env
        )
    ...
    def test_on_complete_with_rejected_promise(self):
        ...
        result = run_lispy_string(
            '(on-complete (reject "error") (fn [p] "cleanup done"))', self.env
        )
    ...
    def test_on_complete_handler_throws_error(self):
        ...
        result = run_lispy_string(
            '(on-complete (resolve "success") (fn [p] (throw "handler failed")))',
            self.env,
        )
    ...
    def test_on_complete_handler_with_rejected_promise_throws(self):
        ...
        result = run_lispy_string(
            '(on-complete (reject "original error") (fn [p] (throw "handler failed")))',
            self.env,
        )
    ...
    def test_on_complete_invalid_promise_type(self):
        ...
        with self.assertRaises(EvaluationError) as cm:
            run_lispy_string(
                '(on-complete "not a promise" (fn [x] "handled"))', self.env
            )
        self.assertEqual(
            str(cm.exception),

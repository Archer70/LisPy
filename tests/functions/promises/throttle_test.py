...
class TestThrottle(unittest.TestCase):
    ...
    def test_throttle_rate_timing_accuracy(self):
        ...
        rate = 200  # 200ms rate
        run_lispy_string(
            f"(define throttled-fn (throttle tracking-fn {rate}))", self.env
        )
    ...
    def test_throttle_with_different_rate_values(self):
        ...
        run_lispy_string(
            "(define throttled-float (throttle tracking-fn 50.5))", self.env
        )
        ...
        run_lispy_string(
            "(define throttled-large (throttle tracking-fn 5000))", self.env
        )
    ...
    def test_throttle_preserves_result_types(self):
        ...
        for i, (func_def, expected) in enumerate(test_cases):
            with self.subTest(func=func_def, expected=expected):
                ...
                run_lispy_string(
                    f"(define {throttled_name} (throttle {func_def} 10))", self.env
                )
    ...
    def test_throttle_rapid_successive_calls(self):
        ...
        self.assertLessEqual(
            self.call_count, 2
        )  # Should be mostly 1, but allow for timing

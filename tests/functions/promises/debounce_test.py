...
class TestDebounce(unittest.TestCase):
    ...
    def test_basic_debounce_behavior(self):
        ...
        run_lispy_string(
            "(define debounced-track (debounce track-calls 100))", self.env
        )
    ...
    def test_debounce_zero_delay(self):
        ...
        run_lispy_string(
            "(define debounced-increment (debounce increment-counter 0))", self.env
        )
    ...
    def test_multiple_debounced_functions(self):
        ...
        run_lispy_string(
            "(define debounced1 (debounce increment-counter1 50))", self.env
        )
        run_lispy_string(
            "(define debounced2 (debounce increment-counter2 50))", self.env
        )
    ...
    def test_thread_safety_concurrent_calls(self):
        ...
        run_lispy_string(
            "(define debounced-increment (debounce increment-call-count 100))", self.env
        )
    ...
    def test_sequential_calls_with_gaps(self):
        ...
        self.env.define("increment-call-count", increment_call_count)
        run_lispy_string(
            "(define debounced-increment (debounce increment-call-count 50))", self.env
        )

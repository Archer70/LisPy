...
class TestPromiseChainingFunctions(unittest.TestCase):
    ...
    def test_then_basic_transformation(self):
        ...
        result = run_lispy_string('(promise-then (resolve 10) (fn [x] (* x 2)))', self.env)
    ...
    def test_then_with_rejected_promise(self):
        ...
        result = run_lispy_string('(promise-then (reject "error") (fn [x] (* x 2)))', self.env)
        self.assertIsInstance(result, LispyPromise)
    ...
    def test_then_argument_validation(self):
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'promise-then' expects 2 arguments (promise callback), got 1.")

        with self.assertRaises(EvaluationError) as cm:
            run_lispy_string('(promise-then (resolve 42) (fn [x] x) "extra")', self.env)
        self.assertEqual(str(cm.exception), "SyntaxError: 'promise-then' expects 2 arguments (promise callback), got 3.")

        # First argument not a promise
        with self.assertRaises(EvaluationError) as cm:
            run_lispy_string('(promise-then 42 (fn [x] x))', self.env)
        self.assertEqual(str(cm.exception), "TypeError: 'promise-then' first argument must be a promise, got int.")

        # Second argument not a function
        with self.assertRaises(EvaluationError) as cm:
            run_lispy_string('(promise-then (resolve 42) "not-a-function")', self.env)
        self.assertEqual(str(cm.exception), "TypeError: 'promise-then' second argument must be a function, got str.")

    def test_on_reject_basic_error_handling(self):
        """Test basic error handling with on-reject."""
        result = run_lispy_string('(on-reject (reject "error") (fn [err] (append "Handled: " (str err))))', self.env)
        self.assertIsInstance(result, LispyPromise)
        self.assertEqual(result.state, "resolved")
    ...
    def test_on_reject_with_resolved_promise(self):
        ...
        result = run_lispy_string('(on-reject (resolve 42) (fn [err] "not called"))', self.env)
        self.assertIsInstance(result, LispyPromise)
        self.assertEqual(result.state, "resolved")
    ...
    def test_on_reject_argument_validation(self):
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'on-reject' expects 2 arguments (promise error-callback), got 1.")

        # First argument not a promise
        with self.assertRaises(EvaluationError) as cm:
            run_lispy_string('(on-reject "not-promise" (fn [x] x))', self.env)
        self.assertEqual(str(cm.exception), "TypeError: 'on-reject' first argument must be a promise, got str.")

        # Second argument not a function
        with self.assertRaises(EvaluationError) as cm:
            run_lispy_string('(on-reject (reject "error") 42)', self.env)
        self.assertEqual(str(cm.exception), "TypeError: 'on-reject' second argument must be a function, got int.")
    ...
    def test_on_complete_with_rejected_promise(self):
        ...
        result = run_lispy_string('(on-complete (reject "error") (fn [p] nil))', self.env)
        self.assertIsInstance(result, LispyPromise)
        self.assertEqual(result.state, "rejected")
        self.assertEqual(result.error, "error")
    ...
    def test_on_complete_argument_validation(self):
        ...
        self.assertEqual(str(cm.exception), "SyntaxError: 'on-complete' expects 2 arguments (promise cleanup-callback), got 1.")

        # First argument not a promise
        with self.assertRaises(EvaluationError) as cm:
            run_lispy_string('(on-complete 42 (fn [x] x))', self.env)
        self.assertEqual(str(cm.exception), "TypeError: 'on-complete' first argument must be a promise, got int.")

        # Second argument not a function
        with self.assertRaises(EvaluationError) as cm:
            run_lispy_string('(on-complete (resolve 42) "not-function")', self.env)
        self.assertEqual(str(cm.exception), "TypeError: 'on-complete' second argument must be a function, got str.")
    ...
    def test_callback_parameter_validation(self):
        ...
        self.assertEqual(str(cm.exception), "TypeError: 'promise-then' callback must take exactly 1 argument, got 0.")
        ...
        with self.assertRaises(EvaluationError) as cm:
            ...
            result = run_lispy_string(code, self.env)
              # Let the promise execute
        self.assertEqual(str(cm.exception), "TypeError: 'on-reject' callback must take exactly 1 argument, got 2.")
        ...
        with self.assertRaises(EvaluationError) as cm:
            ...
            result = run_lispy_string(code, self.env)
              # Let the promise execute
        self.assertEqual(str(cm.exception), "TypeError: 'on-complete' callback must take exactly 1 argument, got 0.")

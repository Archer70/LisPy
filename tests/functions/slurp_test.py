...
class SlurpFunctionTest(unittest.TestCase):
    ...
    def test_slurp_no_arguments(self):
        ...
        self.assertIn("SyntaxError: 'slurp' expects 1 argument (filename), got 0.", str(cm.exception))

    def test_slurp_too_many_arguments(self):
        # Test with too many arguments
        with self.assertRaises(Exception) as cm:
            run_lispy_string('(slurp "file1.txt" "file2.txt")', self.env)
        self.assertIn("SyntaxError: 'slurp' expects 1 argument (filename), got 2.", str(cm.exception))

    def test_slurp_non_string_argument(self):
        # Test with non-string argument
        with self.assertRaises(Exception) as cm:
            run_lispy_string('(slurp 42)', self.env)
        self.assertIn("TypeError: 'slurp' filename must be a string, got int.", str(cm.exception))

    def test_slurp_boolean_argument(self):
        # Test with boolean argument
        with self.assertRaises(Exception) as cm:
            run_lispy_string('(slurp true)', self.env)
        self.assertIn("TypeError: 'slurp' filename must be a string, got bool.", str(cm.exception))
    ...
    def test_slurp_integration_with_split(self):
        ...
        result = run_lispy_string(f'(split (slurp "{escaped_filepath}") "\\n")', self.env)
    ...
    def test_slurp_permission_error(self):
        ...
        with patch('builtins.open', side_effect=PermissionError("Permission denied")):
            ...
            self.assertIn("PermissionError: Permission denied reading file", str(cm.exception))

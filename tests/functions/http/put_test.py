...
class TestHttpPut(unittest.TestCase):
    ...
    def test_http_put_successful_request(self, mock_urlopen):
        ...
        response = run_lispy_string(
            '(await (http-put "https://httpbin.org/put"))', self.env
        )
    ...
    def test_http_put_with_data(self, mock_urlopen):
        ...
        response = run_lispy_string(
            '(await (http-put "https://httpbin.org/put" {:id 123 :name "Updated User"}))',
            self.env,
        )
    ...
    def test_http_put_with_headers(self, mock_urlopen):
        ...
        response = run_lispy_string(
            '(await (http-put "https://httpbin.org/put" nil {:authorization "Bearer token123"}))',
            self.env,
        )

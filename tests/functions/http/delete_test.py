...
class TestHttpDelete(unittest.TestCase):
    ...
    def test_http_delete_successful_request(self, mock_urlopen):
        ...
        response = run_lispy_string(
            '(await (http-delete "https://httpbin.org/delete"))', self.env
        )
    ...
    def test_http_delete_without_body(self, mock_urlopen):
        ...
        response = run_lispy_string(
            '(await (http-delete "https://api.example.com/users/123"))', self.env
        )
    ...
    def test_http_delete_with_data(self, mock_urlopen):
        ...
        response = run_lispy_string(
            '(await (http-delete "https://api.example.com/bulk-delete" {:ids [1 2 3]}))',
            self.env,
        )
    ...
    def test_http_delete_with_headers(self, mock_urlopen):
        ...
        response = run_lispy_string(
            '(await (http-delete "https://api.example.com/resource/456" nil {:authorization "Bearer token123"}))',
            self.env,
        )
    ...
    def test_http_delete_error_responses(self, mock_urlopen):
        ...
        mock_error = HTTPError(
            "https://api.example.com/users/999", 404, "Not Found", {}, None
        )
        ...
        # Make request and await result
        response = run_lispy_string(
            '(await (http-delete "https://api.example.com/users/999"))', self.env
        )
    ...
    def test_http_delete_network_error(self, mock_urlopen):
        ...
        with self.assertRaises(EvaluationError) as context:
            run_lispy_string(
                '(await (http-delete "https://unreachable.example.com/resource"))',
                self.env,
            )

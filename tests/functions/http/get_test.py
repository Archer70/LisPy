...
class TestHttpGet(unittest.TestCase):
    ...
    def test_http_get_successful_request(self, mock_urlopen):
        ...
        mock_response = MockHTTPResponse(
            response_body,
            200,
            {"content-type": "application/json", "server": "test-server"},
        )
        mock_urlopen.return_value = mock_response

        # Make request and await result
        response = run_lispy_string(
            '(await (http-get "https://api.example.com/test"))', self.env
        )
    ...
    def test_http_get_http_error_response(self, mock_urlopen):
        ...
        # Make request and await result
        response = run_lispy_string(
            '(await (http-get "https://api.example.com/missing"))', self.env
        )
    ...
    def test_http_get_network_error(self, mock_urlopen):
        ...
        with self.assertRaises(EvaluationError) as context:
            run_lispy_string(
                '(await (http-get "https://unreachable.example.com"))', self.env
            )
    ...
    def test_http_get_non_json_response(self, mock_urlopen):
        ...
        mock_response = MockHTTPResponse(
            response_body, 200, {"content-type": "text/html"}
        )
        mock_urlopen.return_value = mock_response

        # Make request and await result
        response = run_lispy_string(
            '(await (http-get "https://example.com"))', self.env
        )
    ...
    def test_http_get_invalid_json_response(self, mock_urlopen):
        ...
        # Make request and await result
        response = run_lispy_string(
            '(await (http-get "https://api.example.com/invalid"))', self.env
        )
    ...
    def test_http_get_unicode_response(self, mock_urlopen):
        ...
        mock_response = MockHTTPResponse(
            response_body, 200, {"content-type": "application/json; charset=utf-8"}
        )
        mock_urlopen.return_value = mock_response

        # Make request and await result
        response = run_lispy_string(
            '(await (http-get "https://api.example.com/unicode"))', self.env
        )

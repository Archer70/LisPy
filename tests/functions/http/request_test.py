...
class TestHttpRequest(unittest.TestCase):
    ...
    def test_http_request_method_validation(self):
        ...
        with self.assertRaises(EvaluationError) as context:
            run_lispy_string('(await (http-request 123 "https://httpbin.org/get"))', self.env)
        self.assertIn("Method must be a string", str(context.exception))
        
        # Method cannot be empty
        with self.assertRaises(EvaluationError) as context:
            run_lispy_string('(await (http-request "" "https://httpbin.org/get"))', self.env)
    ...
    def test_http_request_get_method(self, mock_urlopen):
        ...
        response = run_lispy_string('(await (http-request "GET" "https://httpbin.org/get"))', self.env)
    ...
    def test_http_request_post_with_data(self, mock_urlopen):
        ...
        response = run_lispy_string('(await (http-request "POST" "https://httpbin.org/post" {:name "Alice" :email "alice@example.com"}))', self.env)
    ...
    def test_http_request_head_method(self, mock_urlopen):
        ...
        # Make HEAD request
        response = run_lispy_string('(await (http-request "HEAD" "https://httpbin.org/get"))', self.env)
        self.assertEqual(response[':status'], 200)
    ...
    def test_http_request_options_method(self, mock_urlopen):
        ...
        response = run_lispy_string('(await (http-request "OPTIONS" "https://api.example.com/users"))', self.env)
        self.assertEqual(response[':status'], 200)
    ...
    def test_http_request_patch_method(self, mock_urlopen):
        ...
        response = run_lispy_string('(await (http-request "PATCH" "https://api.example.com/users/123" {:status "active"}))', self.env)
    ...
    def test_http_request_custom_method(self, mock_urlopen):
        ...
        response = run_lispy_string('(await (http-request "PURGE" "https://api.example.com/cache"))', self.env)
    ...
    def test_http_request_get_with_data_ignored(self, mock_urlopen):
        ...
        response = run_lispy_string('(await (http-request "GET" "https://httpbin.org/get" {:should "be ignored"}))', self.env)
    ...
    def test_http_request_with_headers(self, mock_urlopen):
        ...
        response = run_lispy_string('(await (http-request "GET" "https://api.example.com/data" nil {:authorization "Bearer token123" :x-custom-header "custom-value"}))', self.env)
    ...
    def test_http_request_error_response(self, mock_urlopen):
        ...
        mock_error = HTTPError("https://api.example.com/missing", 404, "Not Found", {}, None)
        mock_error.read = MagicMock(return_value=error_body.encode('utf-8'))
        mock_error.headers = {}
        mock_urlopen.side_effect = mock_error
        
        # Make request and await result
        response = run_lispy_string('(await (http-request "GET" "https://api.example.com/missing"))', self.env)
    ...
    def test_http_request_network_error(self, mock_urlopen):
        ...
        with self.assertRaises(EvaluationError) as context:
            run_lispy_string('(await (http-request "GET" "https://unreachable.example.com"))', self.env)

...
class EqualsFunctionTest(unittest.TestCase):
    ...
    def test_equals_type_error_non_numeric(self):
        with self.assertRaisesRegex(EvaluationError, r"TypeError: Argument 1 to '=' must be a number for comparison, got str: 'a'"):
            run_lispy_string('(= "a" 5)', self.env)
        with self.assertRaisesRegex(EvaluationError, r"TypeError: Argument 2 to '=' must be a number for comparison, got Vector: '\[.*\]'"):
            run_lispy_string('(= 5 [])', self.env)

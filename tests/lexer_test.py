...
from lispy.lexer import (
    tokenize, TOKEN_NUMBER, TOKEN_STRING, TOKEN_BOOLEAN, TOKEN_NIL, TOKEN_SYMBOL,
    TOKEN_LPAREN, TOKEN_RPAREN, TOKEN_LBRACKET, TOKEN_RBRACKET,
    TOKEN_LBRACE, TOKEN_RBRACE, TOKEN_QUOTE
)
...
class LexerTest(unittest.TestCase):
    ...
    def test_strings_with_escapes(self):
        self.assertEqual(tokenize('"hello\\nworld"'), [
                         (TOKEN_STRING, 'hello\nworld')])
        self.assertEqual(tokenize('"\\"quote\\""'), [
                         (TOKEN_STRING, '"quote"')])
    ...
    def test_string_unterminated_escape(self):
        ...
        with self.assertRaisesRegex(ValueError, r"Lexer error: Unexpected character '\"'"):
            s1 = "\"hello\\\""
            tokenize(s1)
    ...
    def test_whitespace_and_comments(self):
        self.assertEqual(tokenize("  123  "), [(TOKEN_NUMBER, "123")])
        self.assertEqual(tokenize("abc ; this is a comment\n  def"),
                         [(TOKEN_SYMBOL, "abc"), (TOKEN_SYMBOL, "def")])
        self.assertEqual(tokenize("; entire line comment\n456"), [
                         (TOKEN_NUMBER, "456")])
...
"""
    ...
    def test_commas_in_vectors(self):
        ...
        self.assertEqual(tokenize("[1 2 3]"), [
            (TOKEN_LBRACKET, "["),
            (TOKEN_NUMBER, "1"),
            (TOKEN_NUMBER, "2"),
            (TOKEN_NUMBER, "3"),
        ...
        self.assertEqual(tokenize('[1, "two", :three]'), [
            (TOKEN_LBRACKET, "["),
            (TOKEN_NUMBER, "1"),
            (TOKEN_STRING, "two"),
            (TOKEN_SYMBOL, ":three"),
    ...
    def test_commas_in_maps(self):
        ...
        # Map without commas
        self.assertEqual(tokenize("{:a 1 :b 2}"), [
            (TOKEN_LBRACE, "{"),
            (TOKEN_SYMBOL, ":a"),
            (TOKEN_NUMBER, "1"),
            (TOKEN_SYMBOL, ":b"),
        ...
        self.assertEqual(tokenize('{key1: "value1", key2: "value2"}'), [
            (TOKEN_LBRACE, "{"),
            (TOKEN_SYMBOL, "key1:"),
            (TOKEN_STRING, "value1"),
            (TOKEN_SYMBOL, "key2:"),
    ...
    def test_commas_in_function_parameters(self):
        ...
        self.assertEqual(tokenize("(fn [x y z] (+ x y z))"), [
            (TOKEN_LPAREN, "("),
            (TOKEN_SYMBOL, "fn"),
            (TOKEN_LBRACKET, "["),
            (TOKEN_SYMBOL, "x"),

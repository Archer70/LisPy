...
class LispyCompleter(Completer):
    ...
    def __init__(self, environment: Environment):
        self.environment = (
            environment  # The REPL environment, pre-populated with built-ins
        )
    ...
    def get_completions(self, document, complete_event):
        ...
        if (
            not word_to_complete
            and text_before_cursor
            and text_before_cursor[-1] in ("(", "[", "{", " ")
        ):
        ...
        if word_to_complete or provide_all_if_starts_new_symbol:
            ...
            for suggestion in sorted(list(current_suggestions)):
                if provide_all_if_starts_new_symbol or suggestion.startswith(
                    word_to_complete
                ):
...
class LispyRepl:
    ...
    def start_repl(self):
        ...
        print(
            "LisPy Interactive Interpreter (Multi-line, History, Highlighting, Completion, Auto-Indent Guide enabled)"
        )
        print(
            "Type expressions to evaluate. Use Up/Down arrows for history. Press Tab for completions."
        )
        print("Enter an empty line to attempt completion if ambiguous.")
        print(
            "Use (import \"module-name\") to load modules, 'exit' or '(exit)' to quit."
        )
        print()
        ...
        while True:  # Outer loop for each full command
            ...
            if (
                not input_buffer
            ):  # Only reset if buffer was cleared (new command or after processing)
                current_indent_level = 0
            ...
            try:
                ...
                KeyboardInterrupt
            ):  # Ctrl-C: clear current input or exit if buffer empty
                ...
                if (
                    not input_buffer or not "".join(input_buffer).strip()
            ...
            if not input_buffer and line_input.strip().lower() in [
                "exit",
                "quit",
                "(exit)",
                "(quit)",
            ...
            is_complete, open_delims = self._check_buffer_completeness(
                full_input_string
            )
            ...
            if is_complete:
                ...
                should_submit = (
                    (open_delims <= 0)
                    or (line_input.strip() == "" and full_input_string.strip() != "")
                    or (isinstance(line_input, str) and line_input == "")
                )
                ...
                if should_submit:
                    ...
                    if (
                        not final_code_to_run
                    ):  # If it ended up being empty (e.g. multiple blank lines)
                ...
                current_indent_level = max(
                    0, open_delims
                )  # Set indent for the next line
